...
export default function DestinationCard({
  ...
  return (
    <div className={`group relative bg-white border rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between h-auto ${
      isFavorite ? 'border-pink-200 bg-pink-50/5' : 'border-gray-100 hover:border-gray-200/80'
    }`}>
      {/* Destination high resolution imagery */}
      ...
      {/* Main card body with inner padding */}
      <div className="p-4 flex flex-col justify-between flex-1 gap-3">
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center justify-between gap-2">
            {/* Category Tag */}
            <span className={`inline-flex items-center gap-1 text-[11px] font-bold font-mono px-2 py-0.5 rounded-md border ${theme.bg}`}>
              <span>{theme.emoji}</span>
              <span>{destination.category}</span>
            </span>

            {/* Actions (Rating & Favorite toggle) */}
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                title={isFavorite ? 'Hapus dari daftar trip pilihan' : 'Simpan ke daftar trip pilihan'}
              >
        ...
        {/* Description and Informative attributes based on tier */}
        <div className="space-y-3">
          ...
          {userStatus === 'Premium' ? (
            /* Premium Layout: Ultra detailed, clean, precise micro log sheet */
            <div className="bg-slate-50 border border-slate-100/80 rounded-lg p-2.5 space-y-2 select-none">
              <div className="flex items-center gap-1 border-b border-dashed border-slate-200 pb-1 mb-1">
                ...
                <span className="text-[9px] font-extrabold font-mono tracking-wider text-slate-500 uppercase">Premium Travel Guide Log</span>
              </div>
            ...
            /* Free Layout: Simple yet informative visitor summary details */
            <div className="border border-gray-100 bg-gray-50/50 rounded-lg p-2.5 space-y-1.5 text-[11px] leading-relaxed text-gray-500 font-sans">
              <div>
                🌿 <strong>Keunikan:</strong> {destination.uniqueness ? destination.uniqueness.split(',')[0] + '.' : 'Keindahan alam tersembunyi khas lokal.'}
              </div>
              <div className="pt-1.5 border-t border-gray-100 flex flex-col gap-1 text-[10.5px]">
                <div className="truncate" title={destination.access}>
                  🚗 <strong>Akses:</strong> {destination.access ? destination.access.split('.')[0] + '.' : 'Akses jalan aspal pariwisata utama.'}
                </div>
                <div className="truncate" title={destination.openingHours}>
                  🕒 <strong>Jam Buka:</strong> {destination.openingHours ? destination.openingHours.split('(')[0] : 'Setiap Hari.'}
                </div>
              </div>
            </div>
          )}
        ...
        <div className="border-t border-gray-50 pt-2.5 mt-auto flex items-center justify-between">
          <div className="flex flex-col">
            <span className="text-[9px] font-bold font-mono text-gray-400 uppercase tracking-wider">Highlight Utama</span>
